    ...
    methods:{
        init() {
            axios.post('/get_sale_shipment', this.params)
                .then(response => {
                    let sale_shipment = response.data                    
                    this.vat = sale_shipment.vat_amount
                    for (let i = 0; i < sale_shipment.items.length; i++) {
                        const item = sale_shipment.items[i];
                        this.items.push({
